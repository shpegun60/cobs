...
public:
     ...
     */
    inline IRQGuard() {
        primask = __get_PRIMASK();  // Save the current PRIMASK state
     ...
     */
    inline ~IRQGuard() {
        __set_PRIMASK(primask);     // Restore the original PRIMASK state
    }

private:
    uint32_t primask;  // Variable to store the original PRIMASK state
};
