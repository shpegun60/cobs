 ...
 * types for both C and C++ in a consistent and portable way across supported
 * embedded targets. Use them to write clear, explicit code.
 *
 * ----------------------------------------------------------------------------------------------
 ...
 *  Ptr-roundtrip | Pointer <-> integer storage     | reg_ptr, sreg_ptr
 *  Floating      | float/double/long double        | f32, f64, flong, f128*
 *  Size-friendly | API ergonomics for sizes        | usize (size_t), isize (ptrdiff_t)
 *  Char family   | Char + explicit signedness      | c8, sc8, uc8
 *  Misc          | Other types                     | b (bool), uni (void)
 *  Volatile      | MMIO-safe mirrors               | vu32, vi16, vreg, vf32
 * ----------------------------------------------------------------------------------------------
 *  * f128 is a legacy alias of flong and is not guaranteed to be 128-bit.
 *
 ...
 *   - reg_ptr  -> pointer-round-trip integer (uintptr_t)
 ...
 *
 * This file also provides size checks to fail early on unsupported platforms.
 */
...
using f32  = float;
using f64  = double;
using flong = long double; /* extended precision, platform-dependent */
using f128  = flong;       /* legacy alias, not guaranteed 128-bit */
...
typedef float       f32;
typedef double      f64;
typedef long double flong; /* extended precision, platform-dependent */
typedef flong       f128;  /* legacy alias, not guaranteed 128-bit */
...
/* --------------------------- Volatile mirrors ------------------------------- */
/* Volatile aliases are provided for selected non-void aliases: v + <alias>.
...
using vreg_ptr  = volatile reg_ptr;
using vsreg_ptr = volatile sreg_ptr;
...
typedef volatile reg_ptr  vreg_ptr;
typedef volatile sreg_ptr vsreg_ptr;
...
/* ------------------------------ Sanity checks ------------------------------- */
BT_STATIC_ASSERT(CHAR_BIT == 8, "basic_types.h requires 8-bit bytes");
...
BT_STATIC_ASSERT(sizeof(reg_ptr)  == sizeof(void*), "reg_ptr must match pointer size");
BT_STATIC_ASSERT(sizeof(sreg_ptr) == sizeof(void*), "sreg_ptr must match pointer size");

/* Character aliases assume 8-bit chars on supported targets */
BT_STATIC_ASSERT(sizeof(c8)  == 1, "c8 must be 1 byte");
BT_STATIC_ASSERT(sizeof(sc8) == 1, "sc8 must be 1 byte");
BT_STATIC_ASSERT(sizeof(uc8) == 1, "uc8 must be 1 byte");
...
BT_STATIC_ASSERT(sizeof(flong) >= sizeof(f64),
                 "long double must not be narrower than double");

#undef BT_STATIC_ASSERT
#undef BT_CAT
#undef BT_CAT_
