...
Its source hashes are rechecked by `src/wire/tests/verify_hardware_migration.py`.
...
the byte update forced inline (`src/crc/Crc.h`, `CRC_DETAIL_ALWAYS_INLINE`),
...
## Reproduce and compare with silicon
    ...
    python -B src/crc/tests/check_arm_matrix.py
...
Object/ASM files remain in the ignored src/crc/tests/out/all-arm directory.
...
For a short diagnostic subset:
    ...
    python -B src/crc/tests/check_arm_matrix.py --cpus cortex-m0 cortex-m7 cortex-m85 cortex-r52 cortex-a53 arm7tdmi --output src/crc/tests/out/subset
