...
SRC = REPO / "src"
sys.path.insert(0, str(SRC / "crc/tests"))
...
def main() -> None:
    ...
    manifest = {
        ...
        "source_sha256": {
            ...
            for path in (SRC / "crc/Crc.h", SRC / "modbus/rtu/Rtu.h",
                         Path(__file__).with_name("modbus_bench.cpp"),
                         Path(__file__).with_name("build.sh"),
                         *(SRC / "modbus/rtu").glob("*.h"),
                         *(SRC / "modbus/rtu/detail").glob("*.h"),
                         *(SRC / "wire").glob("*.h"),
                         *(SRC / "wire/detail").glob("*.h"))
        },
