...
build test_stream_fuzz "$HERE/test_stream_fuzz.cpp"
build test_layout   "$HERE/test_layout.cpp"
build test_reference_model "$HERE/test_reference_model.cpp"
build_release test_fuzz_o3 "$HERE/test_fuzz.cpp"
build_release test_framing_o3 "$HERE/test_framing.cpp"
build_release test_stream_o3 "$HERE/test_stream.cpp"
build_release test_stream_fuzz_o3 "$HERE/test_stream_fuzz.cpp"
...
"$OUT/test_stream.exe"
"$OUT/test_stream_fuzz.exe"
"$OUT/test_layout.exe"
"$OUT/test_reference_model.exe"

echo "=== random/property and framing suites under -O3/-DNDEBUG ==="
"$OUT/test_fuzz_o3.exe"
"$OUT/test_framing_o3.exe"
"$OUT/test_stream_o3.exe"
"$OUT/test_stream_fuzz_o3.exe"
