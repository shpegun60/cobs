...
def sources():
    ...
    files.add(REPO / "src/wire/tests/extension_checks.h")
...
def main():
    ...
    receipt = dict(schema=3, started=stamp, session=str(session), port=args.port, serial=args.serial,
    ...
    try:
        for protocol, policy, baud in plan:
            ...
            image = dict(tag=tag, protocol=protocol, policy=policy, baud=baud, firmware_version=3,
