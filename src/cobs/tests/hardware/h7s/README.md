...
board using `python -B src/wire/tests/verify_hardware_migration.py` from the root.

From the repository root (these commands build and flash the selected board):

```powershell
powershell -ExecutionPolicy Bypass -File src/cobs/tests/hardware/h7s/run_matrix.ps1 `
...
The geometry, src/wire/control version and numeric tables below describe the
...
described by `src/uart/tests/bench/README.md`. `build.sh` rejects a stale local
...
& 'C:\Program Files\Git\bin\bash.exe' `
  'src/cobs/tests/hardware/h7s/build.sh'
...
python -B src/cobs/tests/hardware/h7s/cobs_hardware.py COM6 `
