...
if ! cmp -s "$PROJECT/Boot/Core/Inc/uart_bench.h" \
             "$SRC/uart/tests/bench/uart_bench.h"; then
...
"$GXX" $CXXFLAGS -DCOBS_HW_BAUD="${COBS_HW_BAUD:-115200}u" \
	...
	${COBS_HW_UART_CHUNK_SIZE:+-DCOBS_HW_UART_CHUNK_SIZE=${COBS_HW_UART_CHUNK_SIZE}u} \
	${COBS_HW_UART_CHUNK_COUNT:+-DCOBS_HW_UART_CHUNK_COUNT=${COBS_HW_UART_CHUNK_COUNT}u} \
  "$HERE/cobs_bench.cpp" -o "$OUT/cobs_bench.o"
OBJS="$OBJS $OUT/cobs_bench.o"

for f in "$SRC/cobs/Decoder.cpp" "$SRC/cobs/Encoder.cpp"; do
