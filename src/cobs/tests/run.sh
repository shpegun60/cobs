...
build() {
	...
	"$CXX" -std=gnu++20 -O1 -g $WARN $CHECKED_STL $SAN -I"$COBS" -I"$HERE" -I"$LIBS/delegate" "$@" -o "$OUT/$name.exe"
}
...
build_release() {
	...
	# shellcheck disable=SC2086
	"$CXX" -std=gnu++20 -O3 -DNDEBUG $WARN $CHECKED_STL -I"$COBS" -I"$HERE" -I"$LIBS/delegate" "$@" -o "$OUT/$name.exe"
}
