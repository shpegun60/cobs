...
def check_crc_table(symbols, linker_map, policy):
    tables = [line for line in symbols.splitlines() if "lookup_" in line]
    assert len(tables) == (1 if policy == 2 else 0), "unexpected CRC table footprint"
    if tables:
        # GNU nm's V says weak object, NOT writable memory. Require the actual
        # flash address, extent AND .rodata input section from the linker map.
        match = re.match(r"(080[0-9a-f]+) 00000200 [rRV] ", tables[0])
        assert match, "table address/size/symbol kind"
        address = int(match[1], 16)
        assert 0x08000000 <= address < address + 512 <= 0x08010000, "table outside boot flash"
        sections = re.findall(r"^ \.rodata\.[^\n]*lookup_[^\n]*\n\s+(0x[0-9a-f]+)\s+(0x[0-9a-f]+)\s", linker_map, re.MULTILINE)
        assert [(int(a, 16), int(n, 16)) for a, n in sections] == [(address, 512)], "table is not in .rodata"
    ...
    for image in receipt["images"]:
        ...
        for relative, sha in image["source_sha256"].items():
            if relative.startswith(("libs/delegate/", "libs/spsc/")):
                ...
                if relative == (HERE / "verify.py").relative_to(REPO).as_posix() and digest(HERE / "verify.py") != sha:
                    # A host-only checker correction does not change measured
                    # firmware. Authenticate its archived bytes; never rewrite
                    # receipt provenance to point at an unmeasured revision.
                    archived = HERE / "verifier_history" / (sha + ".py")
                    assert archived.is_file() and digest(archived) == sha, "missing measured checker version"
                    ordinary[archived.relative_to(REPO).as_posix()] = sha
                else:
        ...
        if local_images:
            ...
            check_crc_table(symbols, (session / (image["tag"] + ".map")).read_text(), c)
