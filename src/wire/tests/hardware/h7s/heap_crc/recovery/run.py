...
"""Exclusive live Heap recovery, with original-flash backup/restore/read-back."""
import argparse
...
def sources():
    ...
    files.update((HERE.parent / "heap.cpp", HERE.parent / "oom/bench.cpp", HERE.parent / "oom/verify.py", REPO / "src/cobs/Encoder.cpp", REPO / "src/cobs/Decoder.cpp",
...
def main():
    ...
    session = PROJECT / "out" / ("heap-recovery-" + stamp)
    session.mkdir()
    ...
    build = PROJECT / "out/heap-recovery"
    ...
    excerpt = "".join(block for block in re.split(r"(?m)(?=^[0-9a-f]+ <)", disassembly)
        if re.match(r"^[0-9a-f]+ <(?:operator new\(|std::get_new_handler\(|wire::Heap::|heap_\w+_probe>|__wrap_abort>|__wrap__exit>|abort>|_exit>)", block))
    ...
    try:
        ...
        for index, key in enumerate(verify.PLAN):
            ...
            with serial.Serial(args.port, 115200, timeout=0.25, write_timeout=3) as port:
                ...
                assert row["hello"].startswith("HELLO,2,600000000,")
                ...
                print(f"{key}: {'CONFIRMED abort -> _exit' if key in verify.FATAL else 'PASS refusal, wire integrity and recovery'}", flush=True)
