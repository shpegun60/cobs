...
test -x "$GXX"
cmp "$PROJECT/Boot/Core/Inc/uart_bench.h" "$SRC/uart/tests/bench/uart_bench.h"
mkdir -p "$OUT"
MCU="-mcpu=cortex-m7 -mfpu=fpv5-d16 -mfloat-abi=hard -mthumb"
INC="-I$PROJECT/Boot/Core/Inc -I$SRC -isystem $LIBS/delegate
...
for f in "$HERE/protocol_bench.cpp" "$SRC/cobs/Decoder.cpp" "$SRC/cobs/Encoder.cpp"; do
