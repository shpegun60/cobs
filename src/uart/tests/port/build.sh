...
COMMON_FLAGS="-c -std=gnu++20 -Os -Wall -Wextra -Wpedantic -Wconversion \
  ...
  -I$HERE -I$SRC/uart -isystem $LIBS/spsc \
  -isystem $LIBS/spsc/src -isystem $LIBS/delegate \
  -isystem $CMSIS_CORE"

OUT="$HERE/out"
mkdir -p "$OUT"
G4_FLAGS="-mcpu=cortex-m4 -mfloat-abi=soft -DSTM32G474xx \
  -I$HERE/g4 -isystem $LIBS/stm32g4xx-hal-driver/Inc \
  -isystem $LIBS/cmsis-device-g4/Include"

echo "=== F1 (STM32F103xE, Cortex-M3, legacy SR/DR) ==="
"$GCC" $COMMON_FLAGS -mcpu=cortex-m3 -mfloat-abi=soft -DSTM32F103xE \
  -I"$HERE/f1" \
  -isystem "$LIBS/stm32f1xx-hal-driver/Inc" \
  -isystem "$LIBS/cmsis-device-f1/Include" \
...
"$GCC" $COMMON_FLAGS -mcpu=cortex-m4 -mfloat-abi=soft -DSTM32G474xx \
  -I"$HERE/g4" \
  -isystem "$LIBS/stm32g4xx-hal-driver/Inc" \
  -isystem "$LIBS/cmsis-device-g4/Include" \
...
echo "=== COBS/RTU adapter integration (real F1/G4/H7RS headers, compile-only) ==="
...
echo "OK (COBS adapter 12 B, RTU adapter 32 B, UART layout unchanged)"
