...
echo "=== freertos-wake ==="
"$CXX" -std=gnu++20 $WARN -D_GLIBCXX_ASSERTIONS -O1 -g   -I"$HERE" -I"$HERE/fake_freertos" -I"$PROJ/uart"   -isystem "$PROJ/libs/spsc" -isystem "$PROJ/libs/spsc/src"   -isystem "$PROJ/libs/delegate"   "$HERE/fake_hal.cpp" "$HERE/test_freertos_wake.cpp" -o "$OUT/freertos-wake.exe"
