...
- `test_cobs_rx` — the assembled RX vertical end to end, plus `PacketRef` semantics, the length-field codec, exact-allocation proof through a recording policy, and every way a declared length can disagree with a frame. Those live here rather than beside the pool because `PacketRef::adopt()` is private to `CobsRx`, its only legitimate source; refcounts are therefore asserted behaviourally, through pool occupancy.
